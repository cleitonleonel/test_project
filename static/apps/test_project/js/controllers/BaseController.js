	...
	methods: {
		get_csrftoken: function(){
 		...
 		check_permission_error: function(data){
 		  ...
 		  try{
 		  	//alert("2");
        if (data.indexOf('ERRO403') == -1) { // Erro por que antes vinha um httpresponse formato de arquivo, dava pra usar esse indexOf, agora vem um json.
        	//alert("3");
          return true;
        }
        //alert("4");
	...
	methods: {
		request: function(url, method, data_paramters, validations, success_function, failure_function){
			...
			if (validations==null || validations()){
				//alert("TO ENVINANDO: "+JSON.stringify(data_paramters))
